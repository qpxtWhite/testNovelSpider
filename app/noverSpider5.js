...
proto.selectOption = function(catalog){
    var ques = ['全部抓取','单卷抓取','合并', '生成epub'], self=this;
    ...
    rl.question(ques, function(answer){
        switch(answer){
            ...
            case '2': self.spideByName(catalog).then(function(){self.selectOption(catalog)}); break;
            case '3': self.concatBook(catalog.map(function(book){return book.bookName;})).then(self.selectOption()); break;
            case '4': self.generateEpub().then(function(){self.selectOption(catalog)}); break;
